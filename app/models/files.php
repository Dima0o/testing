...
class files {
	...
	public function upload ($files,$name,$data=array()){
		$bd = bd::getInstance();
		$user = user::getInstance();
		
		$max_size = 5120; //Максимальный размер в КБ
		...
		if( !is_array($files) )
			return json_encode( array('status' => 'error','text' => 'Файл не выбран.') );
	...
	public function delete_file ($id){
		...
		$file = $bd->getRow('SELECT * FROM `upload_files` WHERE `id`=?i',$id);
		if($file){
			$bd->query('DELETE FROM `upload_files` WHERE `id`=?i',$id);
	...
	public function upload_img ($file,$width,$height){
        ...
        if($response['status'] == 'error')
        	return json_encode( array('status' => 'error','text' => $response['text']) );
		...
		return json_encode( array('status' => 'success','img' => $img ) );
	}
